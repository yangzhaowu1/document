...
全局区-静态全局变量区:全局变量的开辟是在程序在main之前就已经放在内存中。而且对外完全可见。即作用域在全部代码中，任何同包代码均可随时使用，在变量会搞混淆，而且在局部函数中如果同名称变量使用:=赋值会出现编译错误。全局变量最终在进程退出时，由操作系统回收。我们在开发的时候，尽量减少使用全局变量的设计
全局区-常量区：
...
### 进程
* 栈空间从高地址向低地址增长
* 栈大小是固定的，Linux默认为8192K，运行时内存占用超过上限，程序会崩溃；可以通过调大内核参数stack size，或者在创建线程时显式地传入所需大小的内存块
### 协程
 每个goroutine都有自己的栈，栈的初始大小是2KB（g0除外），100万的goroutine会占用2G，但goroutine的栈会在2KB不够用时自动扩容，当扩容为4KB的时候，百万goroutine会占用4GB。
#### 扩容
![栈空间布局](vx_images/301711021220655.png =700x)
* stack.lo: 栈空间的低地址
...
栈扩容流程
1. StackBig的常量，默认为4096，被调用函数栈帧大小大于StackBig的时候， 一定会发生栈的扩容
2. 被调用函数栈帧小于StackSmall：SP小于stackguard0, 执行栈扩容，否则直接执行
3. 被调用函数栈帧大于StackSmall：SP - 函数栈帧大小+ StackSmall 小于stackguard0, 执行栈扩容
栈扩容方式
分段栈（go1.3）：在栈空间不够的时候新申请一个栈空间用于被调用函数的执行， 执行后销毁新申请的栈空间并回到老的栈空间继续执行，当函数出现频繁调用（递归）时可能会引发hot split
连续栈（go1.3之后）：栈空间不足的时候申请一个2倍于当前大小的新栈，并把所有数据拷贝到新栈， 接下来的所有调用执行都发生在新栈上
#### 缩容
目标：提高内存利用率
栈收缩不是在函数调用时发生的，是由垃圾回收器在垃圾回收时主动触发的
流程：计算当前使用的空间，小于栈空间的1/4的话， 执行栈的收缩，将栈收缩为现在的1/2，否则直接返回
缺点：存在栈拷贝和写屏障(write barrier)
